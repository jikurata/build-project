...
class ProjectBuilder {
  ...
  build() {
    ...
    .then(() => this.search())
    .then(() => {
      total = this.buildQueue.length;
      Logger.info(`Starting build process: ${total} files in queue...`);
      return iterator(this.buildQueue);
    })
    .then(() => Logger.info(`Built ${buildCount}/${total} files in ${Date.now() - start}ms.`))
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        while ( paths.length > 0 ) {
          const currpath = paths.shift().replace(/\/+|\\+/g, '/');
  ...
  resolveToDest(path) {
    for ( let src of this.src ) {
      ...
      if ( path.includes(src) ) {
        ...
        if ( !existingmatch ) {
          const resolvedPath = path.replace(regexp, this.dest);
