  ...
  clearDest() {
    return new Promise((resolve, reject) => {
      try {
        ...
        const subpaths = fs.readdirSync(this.dest);
        if ( subpaths.length !== 0 ) {
          Logger.info(`Clearing build destination '${this.dest}' of the following subpaths: '[${subpaths}]...`);
          const paths = [];
          subpaths.forEach(subpath => paths.push(`${this.dest}\\${subpath}`));
          while ( paths.length > 0 ) {
            try {
              const currpath = paths.shift();
              if ( this.isDir(currpath) ) {
                fs.readdirSync(currpath).forEach(p => paths.push(`${currpath}\\${p}`));
                fs.rmdirSync(currpath);
              }
              else if ( this.isFile(currpath) ) fs.unlinkSync(currpath);
            }
            catch(err) { Logger.error(err); }
          }
        }
